...
def _seed_demo_account():
    ...
    profile = LearnerProfile(
        ...
        goal_song="Can't Help Falling in Love",
        goal_event="sister's wedding",
        deadline_date=today + timedelta(days=90),
        minutes_per_day=20,
    ...
    segments = [
        Segment(id="demo-seg-01", skill="Long tones — Bb, C, D (core notes of the song, building tone and breath support)",
                target_date=str(today - timedelta(days=28)), status="complete", confidence=4,
                resource_link=None, content=None),
        Segment(id="demo-seg-02", skill="Opening phrase — 'Wise men say' (Bb, C, D, Eb fingerings)",
                target_date=str(today - timedelta(days=21)), status="complete", confidence=4,
                resource_link=None, content=None),
        Segment(id="demo-seg-03", skill="Second phrase — 'Only fools rush in' (D, C, Bb, G descending line)",
                target_date=str(today - timedelta(days=14)), status="complete", confidence=4,
                resource_link=None, content=None),
        Segment(id="demo-seg-04", skill="Verse melody — first 8 bars of Can't Help Falling in Love",
                target_date=str(today + timedelta(days=7)), status="in_progress", confidence=0,
                resource_link=None, content=None),
        Segment(id="demo-seg-05", skill="D to Eb transition — smooth fingering at tempo",
                target_date=str(today + timedelta(days=14)), status="not_started", confidence=0,
                resource_link=None, content=None),
        Segment(id="demo-seg-06", skill="Breath support — sustaining the long held notes in the chorus",
                target_date=str(today + timedelta(days=28)), status="not_started", confidence=0,
                resource_link=None, content=None),
        Segment(id="demo-seg-07", skill="Bridge section — 'Take my hand' (F, G, F, Eb phrase)",
                target_date=str(today + timedelta(days=42)), status="not_started", confidence=0,
                resource_link=None, content=None),
        Segment(id="demo-seg-08", skill="Articulation and tonguing — soft, legato note attacks for ballad style",
                target_date=str(today + timedelta(days=56)), status="not_started", confidence=0,
                resource_link=None, content=None),
        Segment(id="demo-seg-09", skill="Full song at half tempo — consistent intonation and phrasing",
                target_date=str(today + timedelta(days=70)), status="not_started", confidence=0,
                resource_link=None, content=None),
        Segment(id="demo-seg-10", skill="Full song performance run-through at wedding tempo",
                target_date=str(today + timedelta(days=84)), status="not_started", confidence=0,
                resource_link=None, content=None),
    ]
    ...
    plan = Plan(
        ...
        last_replan_reason="Added D to Eb transition segment after difficulty ratings showed consistent struggle with that interval",
    )
    save_plan(plan)

    # Seed 7 days of realistic history
    entries = [
        (7, 3, 0.33, "behind",   8, "D to Eb transition — fingers feel slow and the note cuts out", "demo-seg-01"),
        (6, 3, 0.67, "behind",   7, "timing on the second phrase, keep rushing the descending line", "demo-seg-02"),
        (5, 4, 0.67, "on_track", 6, "breath support runs out before the long held notes end",        "demo-seg-02"),
        (4, 4, 1.00, "on_track", 5, None,                                                            "demo-seg-03"),
        (3, 4, 0.67, "on_track", 6, "Eb fingering still inconsistent when playing up to tempo",      "demo-seg-03"),
...
async def seed_demo_history(learner_id: str):
    ...
    entries = [
        (7, 3, 0.33, "behind",   8, "D to Eb transition — fingers feel slow and the note cuts out"),
        (6, 3, 0.67, "behind",   7, "timing on the second phrase, keep rushing the descending line"),
        (5, 4, 0.67, "on_track", 6, "breath support runs out before the long held notes end"),
        (4, 4, 1.00, "on_track", 5, None),
        (3, 4, 0.67, "on_track", 6, "Eb fingering still inconsistent under tempo"),
